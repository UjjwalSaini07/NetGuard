...
export default function ScanTriggerForm({ onClose, onScanComplete }) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-3 sm:p-4 overflow-y-auto animate-in fade-in duration-150">
      <div className="glass-card relative w-full max-w-lg max-h-[92vh] flex flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white p-4 sm:p-6 md:p-8 shadow-2xl my-auto">
        <div className="mb-4 sm:mb-6 flex items-start justify-between shrink-0">
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            <div className={`flex h-10 w-10 sm:h-11 sm:w-11 shrink-0 items-center justify-center rounded-2xl border shadow-sm ${
              scanResult
                ? 'bg-emerald-50 border-emerald-200 text-emerald-600'
                : 'bg-indigo-50 border-indigo-100 text-indigo-600'
            }`}>
              {scanResult ? <HiCheckCircle className="h-5 w-5 sm:h-6 sm:w-6" /> : <HiBolt className="h-5 w-5 sm:h-6 sm:w-6" />}
            </div>
          ...
          {!submitting && (
            <button
              onClick={onClose}
              className="rounded-xl p-1.5 sm:p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-colors shrink-0 ml-1"
            >
        ...
        {scanResult ? (
          <div className="space-y-6">
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-2.5 sm:gap-3 pt-3 border-t border-slate-100">
              <button
                type="button"
                onClick={() => setScanResult(null)}
                className="flex items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition-colors w-full sm:w-auto whitespace-nowrap"
              >
                <HiArrowPath className="h-3.5 w-3.5" />
                <span className="whitespace-nowrap">Run Another Scan</span>
              </button>
              <button
                type="button"
                onClick={onClose}
                className="flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 sm:px-6 py-2.5 text-xs font-bold uppercase tracking-wider text-white shadow-sm shadow-indigo-200 transition-all hover:bg-indigo-700 active:scale-[0.98] w-full sm:w-auto whitespace-nowrap"
              >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between gap-2 mb-1.5 min-w-0">
                <span className="text-[11px] font-semibold text-slate-700 truncate">
                  Public Cloud Targets (AWS Lambda):
                </span>
                <span className="text-[10px] text-indigo-600 font-medium shrink-0 whitespace-nowrap">
                  <span className="hidden sm:inline">Accessible Worldwide</span>
              ...
              <div className="flex items-center justify-between gap-2 mb-1.5 min-w-0">
                <span className="text-[11px] font-semibold text-slate-500 truncate">
                  Local LAN Targets (Dev Mode):
                </span>
                <span className="text-[10px] text-slate-400 font-medium shrink-0 whitespace-nowrap">
                  <span className="hidden sm:inline">Local Machine & Wi-Fi</span>
                  <span className="sm:hidden">Local LAN</span>
                </span>
              </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center sm:justify-end gap-2.5 sm:gap-3 pt-3 border-t border-slate-100">
              <button
                type="button"
                onClick={onClose}
                className="rounded-xl px-4 py-2.5 text-xs font-semibold text-slate-500 hover:text-slate-800 transition-colors w-full sm:w-auto text-center"
              >
                Cancel
              </button>
              <button
                type="submit"
                disabled={submitting}
                className="flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-6 py-2.5 text-xs font-bold uppercase tracking-wider text-white shadow-sm shadow-indigo-200 transition-all hover:bg-indigo-700 active:scale-[0.98] disabled:opacity-50 w-full sm:w-auto whitespace-nowrap"
              >
                <HiBolt className="h-4 w-4" />
                <span className="whitespace-nowrap">Launch Scan</span>
              </button>
            </div>
          </form>
        )}
